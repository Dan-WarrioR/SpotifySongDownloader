namespace SpotifyDownloader.Scripts.Features.YouTubeMusic
{
    public class YtmDownloadState
    {
        ...
        public List<YtmDownloadResult> Results { get; set; } = [];
    }

    public class YtmDownloadResult
    {
    ...
    public class YtmDownloadStateManager
    {
        private readonly Lock _lock = new();
        private YtmDownloadState _state = new();

        public YtmDownloadState GetState()
        {
            lock (_lock)
            {
                return new YtmDownloadState
                {
                    ...
                    Results = new List<YtmDownloadResult>(_state.Results)
                };
            }
        }
        ...
        public void AddSuccess(string track, string artist)
        {
            ...
            {
                _state.Completed++;
                _state.Results.Add(new YtmDownloadResult { Track = track, Artist = artist, Success = true });
                UpdateProgress();
            }
        }
        ...
        public void AddFailure(string track, string artist, string error)
        {
            ...
            {
                _state.Failed++;
                _state.Results.Add(new YtmDownloadResult { Track = track, Artist = artist, Success = false, Error = error });
                UpdateProgress();
            }
        }
